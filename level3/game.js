class Game {
    ...
    createBadPlane(num) {
        for (let i = 0; i < num; i++) {
            let x = this.randomNumber(0, paper.width - 30);
            let Src = '../image/darkPlane.png'
            let y = this.randomNumber(-70, 0)
            let badPlane = new BadPlane(x, y, Src);
            badPlane.display(paper);
            badPlanes.push(badPlane);
        }
    }
    displayBadPlanes() {
        for (let i = 0; i < badPlanes.length; i++) {
            badPlanes[i].display(paper);
    ...
    moveBadPlanes() {
        ...
        for (let i = 0; i < badPlanes.length; i++) {
            ...
            badPlanes[i].moveDown()
        }
    }
    moveBadPlanes2(){
         for (let j = 0; j < badPlanes.length; j++) {
            badPlanes[j].move()
        }
    }
    badPlanesFire() {
        for (let i = 0; i < badPlanes.length; i++) {
            badPlanes[i].autoFire()
        }
    }
    displayPlayer() {
    ...
    checkCrash(){
        for (let i = 0; i < bullets.length; i++) {
           if(this.ifCrash(bullets[i],this.player,6,45)==true){
               bullets.splice(i,1)
               this.downHp(10)
           }
        ...
        for (let i = 0; i < rocks.length; i++) {
         ...
         for (let i = 0; i < bullets2.length; i++) {
          for(let j=0; j<badPlanes.length;j++){
             if(this.ifCrash(bullets2[i],badPlanes[j],10,45)==true){
                 this.crash.play()
                 bullets2.splice(i,1)
                 this.createBoom(1,badPlanes[j].x,badPlanes[j].y)
                 badPlanes.splice(j,1)
